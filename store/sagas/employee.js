...
// ADDING THIS
function* deleteAllEmployee(action) {
	try {
		const response = yield fetch("/api/employees/" + action.payload, {
			method: "DELETE ALL",
		});

		const deletedAllEmployee = yield response.json();

		yield put({
			type: t.EMPLOYEE_DELETE_ALL_SUCCEEDED,
			payload: deletedAllEmployee.data.id
		});
	} catch (error) {
		yield put({
			type: t.EMPLOYEE_DELETE_ALL_FAILED,
			payload: error.message,
		});
	}
}
...
function* watchRemoveAllEmployee() {
	yield takeLatest(t.EMPLOYEE_DELETE_ALL_REQUESTED, deleteAllEmployee);
}
...
export default function* rootSaga() {
	yield all([
		watchFetchEmployees(),
		watchAddEmployee(),
		watchRemoveAllEmployee(), // ADDING THIS
		watchRemoveEmployee(),
		watchUpdateEmployee(),
	]);
}
